    ...
    <div className="mx-auto max-w-screen-2xl px-4 py-10 lg:px-8">
      ...
      <div className="surface-panel relative mb-6 overflow-hidden px-5 py-4 sm:px-6 sm:py-5">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-3">
          <div className="min-w-0 flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
            ...
            <span className="text-xs text-[var(--muted)]">· Spiritual talks by HH Haladhara Svāmī in {label}</span>
          </div>
          <div className="chip flex shrink-0 items-center gap-1.5 px-3 py-1.5 text-xs font-semibold text-[var(--accent)]">
            🎙️ {playlists.length} playlist{playlists.length !== 1 ? "s" : ""}
          </div>
        </div>
      </div>
      ...
      <div className="lg:grid lg:grid-cols-[1fr_260px] lg:items-start lg:gap-10">
        <div className="min-w-0">
          <form action={`/audio/${lang}`} method="GET" className="mb-6 flex max-w-md gap-2">
                  ...
                  <Link href={`/audio/playlist/${pl.id}`} className="block">
                    ...
                    <div className="p-4 pr-12">
                      ...
                      <p className="mt-1 text-xs text-[var(--muted)]">
                        Playlist · {pl._count.lectures} lecture{pl._count.lectures !== 1 ? "s" : ""}
                      </p>
                    </div>
                  </Link>
        ...
        <aside className="hidden lg:sticky lg:top-24 lg:block">
          <div className="surface-panel p-4">
            ...
            <div className="flex flex-col gap-1">
              <Link href={`/audio/${lang}`} data-active={!tag} className="filter-item block px-3 py-2 text-sm">
                All
              </Link>
              {categories.map((category) => (
                <Link
                  key={category.id}
                  href={`/audio/${lang}?tag=${category.slug}`}
