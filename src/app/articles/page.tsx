...
export default async function ArticlesPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10">
      <PageHeader eyebrow="Reflections & teachings" title="Articles" />

      {articles.length === 0 ? (
        <div className="empty-state py-16 text-center text-sm text-[var(--muted)]">No articles have been published yet.</div>
      ) : (
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
          {articles.map((article) => (
            <Link
              ...
              className="surface-card group overflow-hidden transition-transform hover:-translate-y-0.5"
            >
              {article.coverUrl && (
              ...
              <div className="p-5">
                <h2 className="text-lg font-semibold leading-tight text-[var(--foreground)] group-hover:text-[var(--accent)]">
                  {article.title}
                </h2>
