...
export function FullScreenPlayer() {
  ...
  const [view, setView] = useState<"player" | "notes">("player")
  ...
  useEffect(() => {
    ...
    function draw() {
      ...
      for (let i = 0; i < bars; i++) {
        ...
        // read the live theme accent so the waveform matches light/dark mode
        const accent = getComputedStyle(cv).getPropertyValue("--accent").trim() || "#4A5D8F"
        ...
        >
          ...
          <div className="flex items-center justify-between px-5 pt-12 pb-4">
            <button onClick={closeFullScreen} aria-label="Close player">
              <X size={24} className="text-[var(--foreground)]/70" />
            </button>
            {session ? (
              <div className="flex rounded-full border border-[var(--border)] p-0.5">
                <button
                  ...
                  style={view === "player" ? { background: "var(--accent)", color: "var(--accent-fg)" } : { color: "var(--muted)" }}
                >
                  Player
                </button>
                <button
                  ...
                  className="rounded-full px-3 py-1 text-[11px] font-semibold uppercase tracking-widest transition-colors"
                  style={view === "notes" ? { background: "var(--accent)", color: "var(--accent-fg)" } : { color: "var(--muted)" }}
                >
                  Notes
                </button>
              </div>
          ...
          {session && (
            <div
              ...
              style={{ display: view === "notes" ? "block" : "none" }}
            >
              <NotebookOverlay
...
function SortableQueueItem({ track, onRemove }: { track: Track; onRemove: () => void }) {
  ...
  return (
    <div
      ...
      className="flex items-center gap-2 rounded-lg p-1.5 hover:bg-[var(--foreground)]/5"
    >
      <span {...attributes} {...listeners} className="cursor-grab text-[var(--foreground)]/30 px-1 text-sm">
        ⠿
      </span>
      <p className="flex-1 truncate text-xs text-[var(--foreground)]/70">{track.title}</p>
      <button onClick={onRemove} className="shrink-0 text-[var(--foreground)]/30 hover:text-[var(--foreground)]/70">
        <X size={12} />
      </button>
    </div>
  )
