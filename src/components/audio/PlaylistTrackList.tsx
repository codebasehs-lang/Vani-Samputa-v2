...
import { Play, Pause, Plus } from "phosphor-react"
...
export function PlaylistTrackList({
  ...
  return (
    <div>
      ...
      <button
        onClick={playAll}
        className="mb-4 flex items-center gap-2 rounded-full px-5 py-2 text-sm font-semibold text-white transition-opacity hover:opacity-90"
      ...
      <div className="flex flex-col divide-y divide-[var(--border)]">
        {lectures.map((lecture, idx) => {
          ...
          return (
            <div
              key={lecture.id}
              className={`group flex items-center gap-3 py-3 ${isActive ? "text-[var(--saffron)]" : "text-[var(--foreground)]"}`}
            >
              ...
              <div className="flex w-6 shrink-0 justify-center">
                ...
                {!isActive && (
                  <button
                    ...
                    className="hidden group-hover:block text-[var(--saffron)]"
                  >
                    <Play size={16} weight="fill" />
                  </button>
                )}
              ...
              <div className="min-w-0 flex-1">
                <p className={`truncate text-sm font-medium ${isActive ? "text-[var(--saffron)]" : ""}`}>
                  {lecture.title}
                </p>
                {lecture.duration && (
