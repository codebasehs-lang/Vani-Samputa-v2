...
async function main() {
  ...
  const categoryNames = [
    ...
    "Festival Lectures", "Vaishnava Songs & Bhajans", "Question & Answer Sessions",
    "Chanting Suddha Nama", "Guru Tattva", "Seva", "Goswami", "General",
  ]
  ...
  const oldPlaylists = await prisma.playlist.findMany({ select: { id: true, category: true } })
  for (const playlist of oldPlaylists) {
    await prisma.playlist.update({ where: { id: playlist.id }, data: { categories: { connect: { name: playlist.category } } } })
  }
  const oldLectures = await prisma.lecture.findMany({ select: { id: true, category: true } })
  for (const lecture of oldLectures) {
    await prisma.lecture.update({ where: { id: lecture.id }, data: { categories: { connect: { name: lecture.category } } } })
  }
  ...
  const aboutArticles = [
    {
      ...
      coverUrl: "/gurudeva-photo/Gurudeva1.png",
    },
    {
      ...
      coverUrl: "/gurudeva-photo/Gurudeva2.png",
    },
    {
      ...
      coverUrl: "/gurudeva-photo/Gurudeva3.png",
    },
  ]
